...
5. ~~**`SnapshotService`** — snapshot/resume. Needs read access to the other collaborators' state; define an explicit snapshot interface per collaborator rather than letting it reach into their fields.~~ **DONE** (2026-07-05). Owns serialization/framing/fail-closed validation for `snapshotSparse`/`restoreSparse`/`snapshot`/`restoreInto`; `EntityAllocator` rides in whole as its own snapshot seam (step 3's interface); Store keeps the `DETERMINISM_DISABLED` gates and the live-world mutation seams (`_mountRestoredDense` — the only `_columnStore` assignment — and `_reconstructHostRows`). Cold path, no bench gate per §Verification; suite + snapshot round-trip tests green.
6. ~~**`ArchetypeGraph`** — archetype graph management. Most entangled with hot paths; do last, benchmark before/after.~~ **DONE** (2026-07-05, bench-gated). Boundary: the graph owns *topology* (archetype list, hash-bucketed mask map, id counter, inverted `componentIndex`, edge resolution/creation + the ADR-0015 install invariant, `storeSpecFromLayouts`); *storage lifecycle* stays on `Store` as creation-path-only closure seams (`_archExtendStoreWithNewSpecs` = SAB extend + view refresh, `_materializeArchetype` = column-store binding + grow handler, `_fanIntoQueries` = query-registry fan-in) — an edge-cache hit never calls the host. A/B: registration 1.03x vs control 1.01x, all other groups band-identical; a scary-looking `fragmentation: pathological` 0.72x did not survive a quiet re-run (1.02x — the A side itself had dropped 23% under 13% rme, pure machine noise). Bench session also restored `registration`/`query`/`ref_iteration` + the compose 4-shape cases, which had been silently dead since the 0.4 port (`.not`→`.without`, `.any_of`→`.anyOf`, `register_system`→`registerSystem`, 256 components > the 128 dense cap → 120).
