...
describe("relations wildcard — (R, *) over an exclusive relation", () => {
	...
	it("enumerates every (source, target) pair", () => {
		...
		const targets = ecs.relations.register();
		ecs.relations.add(ents[0], targets, ents[3]);
		ecs.relations.add(ents[1], targets, ents[2]);
		ecs.relations.add(ents[2], targets, ents[3]);
		expect(ecs.relations.pairsOf(targets)).toEqual([
			[ents[0], ents[3]],
			[ents[1], ents[2]],
			[ents[2], ents[3]]
	...
	it("reflects only the current target after a re-target (one-per-source)", () => {
		...
		const targets = ecs.relations.register();
		ecs.relations.add(ents[0], targets, ents[1]);
		ecs.relations.add(ents[0], targets, ents[2]); // overwrites
		expect(ecs.relations.pairsOf(targets)).toEqual([[ents[0], ents[2]]]);
	});
	...
	it("drops a pair once the source's relation is removed", () => {
		...
		ecs.relations.add(ents[0], targets, ents[1]);
		ecs.relations.add(ents[1], targets, ents[2]);
		ecs.relations.remove(ents[0], targets);
		expect(ecs.relations.pairsOf(targets)).toEqual([[ents[1], ents[2]]]);
	});
});
...
describe("relations wildcard — (R, *) over a multi relation", () => {
	...
	it("expands a source's whole target set, targets ascending", () => {
		const { ecs, ents } = makeWorld(4);
		const likes = ecs.relations.register({ multi: true });
		ecs.relations.add(ents[0], likes, ents[3]);
		ecs.relations.add(ents[0], likes, ents[1]);
		ecs.relations.add(ents[0], likes, ents[2]);
		expect(ecs.relations.pairsOf(likes)).toEqual([
			[ents[0], ents[1]],
			[ents[0], ents[2]],
			[ents[0], ents[3]]
	...
	it("drops a source from enumeration once its last target is removed", () => {
		...
		const likes = ecs.relations.register({ multi: true });
		ecs.relations.add(ents[0], likes, ents[1]);
		ecs.relations.add(ents[1], likes, ents[2]);
		ecs.relations.remove(ents[0], likes, ents[1]); // empties ents[0]'s set
		expect(ecs.relations.pairsOf(likes)).toEqual([[ents[1], ents[2]]]);
	});
});
...
describe("relations wildcard — (*, T) across all relation kinds", () => {
	it("returns no sources when nothing targets T", () => {
		...
		expect(ecs.relations.sourcesOfAny(ents[1])).toEqual([]);
	});
	...
	it("collects sources of T from a single relation", () => {
		...
		const targets = ecs.relations.register();
		ecs.relations.add(ents[0], targets, ents[3]);
		ecs.relations.add(ents[1], targets, ents[3]);
		ecs.relations.add(ents[2], targets, ents[0]); // unrelated target
		expect(ecs.relations.sourcesOfAny(ents[3])).toEqual([
			[targets, ents[0]],
			[targets, ents[1]]
		]);
	});
	...
	it("spans every relation kind, ordered by relation id then source id", () => {
		...
		const targets = ecs.relations.register(); // relation id 0 (exclusive)
		const likes = ecs.relations.register({ multi: true }); // relation id 1
		// T = ents[4]. Sources across both relations.
		ecs.relations.add(ents[2], targets, ents[4]);
		ecs.relations.add(ents[0], targets, ents[4]);
		ecs.relations.add(ents[3], likes, ents[4]);
		ecs.relations.add(ents[1], likes, ents[4]);
		expect(ecs.relations.sourcesOfAny(ents[4])).toEqual([
			[targets, ents[0]],
			[targets, ents[2]],
			[likes, ents[1]],
			[likes, ents[3]]
...
describe("relations wildcard — determinism", () => {
	...
	it("(R, *) folds scrambled inserts to canonical source order (exclusive)", () => {
		const { ecs, ents } = makeWorld(5);
		const r = ecs.relations.register();
		// Sources inserted out of index order: 3, 0, 4, 1.
		ecs.relations.add(ents[3], r, ents[0]);
		ecs.relations.add(ents[0], r, ents[2]);
		ecs.relations.add(ents[4], r, ents[1]);
		ecs.relations.add(ents[1], r, ents[3]);
	...
	it("(R, *) folds scrambled inserts to canonical source+target order (multi)", () => {
		const { ecs, ents } = makeWorld(5);
		const r = ecs.relations.register({ multi: true });
		// Both source order and per-source target order scrambled.
		ecs.relations.add(ents[3], r, ents[4]);
		ecs.relations.add(ents[0], r, ents[3]);
