...
- [ ] **Static query resolution** — Pre-resolve queries at init time and maintain the archetype list incrementally. Drop the fingerprint validation on every `ctx.query()` call.
...
- [ ] **`Object.freeze` on archetypes and queries** — Cheap to add, signals immutability to V8.
